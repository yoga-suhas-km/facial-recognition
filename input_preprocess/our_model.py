...
from model_global_variable import IMAGE_SIZE_V, IMAGE_SIZE_H, num_classes
...
def generate_model():
    ...
    """
    # Block 1
    model.add(Conv2D(64, (3, 3), activation='relu', padding='same', name='conv1_1',input_shape=(IMAGE_SIZE_V, IMAGE_SIZE_H, 1)))
...
def train(model, x_train, x_test, y_train, y_test):
    ...
    while epoch_count < model_global_variable.epoch:
        ...
        model.fit(x_train, y_train, verbose=1, epochs=1, batch_size=model_global_variable.batch_size, validation_data=(x_test, y_test))
...
def print_performance_metrics(model, x_test, y_test, batch_size):
    ...
    y_pred_bool = np.argmax(x_pred_t, axis=1)
